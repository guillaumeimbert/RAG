...
-- and then sorts. The schema's half-precision mirror (embedding_hv) lifts the
-- HNSW cap to 4000 dims, so candidate retrieval becomes an ordered Index Scan.
-- This script measures the difference at the reference 2560-dim width.
--
...
CREATE INDEX bench_chunks_hnsw
    ON bench_chunks USING hnsw (embedding_hv halfvec_cosine_ops);
...
SELECT count(*) FROM (SELECT id FROM bench_chunks ORDER BY embedding_hv <=> (:qv) LIMIT 5) w;
SELECT count(*) FROM (SELECT id FROM bench_chunks ORDER BY embedding_hv <=> (:qv) LIMIT 5) w;

\timing on
\echo '=== INDEXED: HNSW Index Scan on embedding_hv (enable_seqscan=off) ==='
SET enable_seqscan = off;
SELECT id FROM bench_chunks ORDER BY embedding_hv <=> (:qv) LIMIT 5;
SELECT id FROM bench_chunks ORDER BY embedding_hv <=> (:qv) LIMIT 5;
SELECT id FROM bench_chunks ORDER BY embedding_hv <=> (:qv) LIMIT 5;
