...
- Purpose: temporary OCaml 5.5 compatibility bridge while Alanvi migrates to
  direct Caqti requests.
...
The root `dune` file builds only the PPX, runtime, and Lwt adapter used by
Alanvi. Upstream examples, tests, Async support, and Eio support remain in the
import for provenance but are excluded from Alanvi's build graph.
